    ...
    class RBMBehaviorArcherSkirmish : BehaviorComponent
    {
        ...
        protected override void CalculateCurrentOrder()
        {
            ...
            else
            {
                ...
                {
                    if (_currentTacticField.GetValue(base.Formation?.Team?.TeamAI) != null 
                        && _currentTacticField.GetValue(base.Formation?.Team?.TeamAI).ToString().Contains("SplitArchers"))
                    {
                    ...
                    case BehaviorState.Shooting:
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Formation meleeFormation = RBMAI.Utilities.FindSignificantAlly(base.Formation, true, false, false, false, false);
                                    var mediandist = meleeFormation.QuerySystem.MedianPosition.AsVec2.Distance(base.Formation.QuerySystem.MedianPosition.AsVec2);
                                    if (meleeFormation != null 
                                        && meleeFormation.QuerySystem.IsInfantryFormation 
                                        && mediandist <= base.Formation.QuerySystem.MissileRange)
                                    {
                                        rollPullBackAngle = MBRandom.RandomFloat;
                                        _behaviorState = BehaviorState.PullingBack;
                                        break;
                    ...
                    case BehaviorState.PullingBack:
                        {
                            ...
                            var mediandist = meleeFormationPull.QuerySystem.MedianPosition.AsVec2.Distance(base.Formation.QuerySystem.MedianPosition.AsVec2);
                            if (meleeFormationPull != null && mediandist > base.Formation.QuerySystem.MissileRange)
                            {
                                _behaviorState = BehaviorState.Shooting;
                                flag = true;
                            }
                ...
                if (firstTime || previousBehavior != _behaviorState || shouldReposition)
                {
                    switch (_behaviorState)
                    {
                        ...
                        case BehaviorState.Approaching:
                            ...
                            if (side == 0)
                            {
                                medianPosition.SetVec2(
                                    significantEnemy.QuerySystem.AveragePosition 
                                    + significantEnemy.QuerySystem.MedianPosition.AsVec2.LeftVec().Normalized() * rollPullBackAngle * 70f);
                            }
                            else if (side == 1)
                            {
                                medianPosition.SetVec2(
                                    significantEnemy.QuerySystem.AveragePosition 
                                    + significantEnemy.QuerySystem.MedianPosition.AsVec2.RightVec().Normalized() * rollPullBackAngle * 70f);
                            }
                            break;
                        case BehaviorState.PullingBack:
                            ...
                            {
                                medianPosition.SetVec2(
                                    (medianPosition.AsVec2 - vec * (effectiveShootingRange - base.Formation.Depth * 0.5f)) 
                                    + significantEnemy.QuerySystem.MedianPosition.AsVec2.LeftVec().Normalized() * rollPullBackAngle * 70f);
                            }
                            else if (side == 1)
                            {
                    ...
                    if (!CurrentFacingOrder.GetDirection(base.Formation).IsValid || _behaviorState != BehaviorState.Shooting || flag)
                    {
                        ...
                        CurrentFacingOrder = FacingOrder.FacingOrderLookAtDirection(
                            (medianTargetFormationPosition.AsVec2 - base.Formation.QuerySystem.AveragePosition).Normalized());
                    }
                    firstTime = false;
                }
            }
