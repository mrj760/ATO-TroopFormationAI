...
public class SiegeArcherPoints : MissionView
{
    ...
    public override void OnMissionScreenTick(float dt)
    {
        ...
        if (isFirstTimeLoading && Mission.Current != null && Mission.Current.IsSiegeBattle)
        {
            ...
            if (File.Exists(Utilities.GetSiegeArcherPointsPath() + Mission.Current.Scene.GetName() + "_" +
                            Mission.Current.Scene.GetUpgradeLevelMask() + ".xml"))
            {
                xmlDocument.Load(Utilities.GetSiegeArcherPointsPath() + Mission.Current.Scene.GetName() + "_" +
                                 Mission.Current.Scene.GetUpgradeLevelMask() + ".xml");
                xmlExists = true;
            }
            ...
            if (xmlExists)
            {
                ...
                var _strategicAreas = (from amo in Mission.Current.ActiveMissionObjects
                    where (strategicArea = amo as StrategicArea) != null && strategicArea.IsActive &&
                ...
                var ListBase = Mission.Current.Scene.FindEntitiesWithTag("BeerMarkerBase").ToList();
                ...
                var ListG = Mission.Current.Scene.FindEntitiesWithTag("PlayerStratPoint").ToList();
                var ListArrow = Mission.Current.Scene.FindEntitiesWithTag("BeerMarkerPlayer").ToList();
                ...
                foreach (XmlNode pointNode in xmlDocument.SelectSingleNode("/points").ChildNodes)
                {
                    ...
                    var gameEntity = GameEntity.Instantiate(Mission.Current.Scene, "strategic_archer_point", matFrame);
        ...
        if (firstTime && Mission.Current != null && Mission.Current.IsSiegeBattle &&
            Mission.Current.PlayerTeam.IsDefender && Mission.Current.Mode != MissionMode.Deployment)
        {
            if (firstTime && devmode)
            {
                firstTime = false;
                return;
            }
            ...
            InformationManager.DisplayMessage(
                new InformationMessage("!!! DEVELOPER MODE, NORMAL USER SHOULDN'T SEE THIS MESSAGE"));
            var gameEntities = new List<GameEntity>();
            Mission.Current.Scene.GetEntities(ref gameEntities);
            ...
            var pointNode = xmlDocument.SelectSingleNode("/points");
            if (pointNode == null) pointNode = xmlDocument.CreateElement(string.Empty, "points", string.Empty);
            ...
            foreach (var g in gameEntities)
                    ...
                    g.GetOldPrefabName() == "strategic_archer_point")
                {
                    ...
                    var stringToBeSaved = "";
                    stringToBeSaved += g.GetGlobalFrame().rotation.s.x + "," + g.GetGlobalFrame().rotation.s.y + "," +
                                       g.GetGlobalFrame().rotation.s.z + ",";
                    stringToBeSaved += g.GetGlobalFrame().rotation.f.x + "," + g.GetGlobalFrame().rotation.f.y + "," +
                                       g.GetGlobalFrame().rotation.f.z + ",";
                    stringToBeSaved += g.GetGlobalFrame().rotation.u.x + "," + g.GetGlobalFrame().rotation.u.y + "," +
                                       g.GetGlobalFrame().rotation.u.z + ",";
                    stringToBeSaved += g.GetGlobalFrame().origin.x + "," + g.GetGlobalFrame().origin.y + "," +
                                       g.GetGlobalFrame().origin.z;
                    newPointNode.InnerText = stringToBeSaved;

                    pointNode.AppendChild(newPointNode);
                }

            xmlDocument.Save(Utilities.GetSiegeArcherPointsPath() + Mission.Current.Scene.GetName() + "_" +
                             Mission.Current.Scene.GetUpgradeLevelMask() + ".xml");
            firstTime = false;
        }
    ...
    private class GetCloseStrategicAreasPatch
    {
            ...
            ref ArrangementOrder __instance)
        {
            ...
            {
                var customDistanceToCheck = 150f;
                if (sa != null && formation != null && sa.GameEntity != null && sa.GameEntity.GlobalPosition != null &&
                    sa.IsUsableBy(formation.Team.Side))
                {
                    if (sa.IgnoreHeight)
                    {
                    ...
                    return worldPosition.DistanceSquaredWithLimit(in targetPoint,
                               customDistanceToCheck * customDistanceToCheck + 1E-05f) <
